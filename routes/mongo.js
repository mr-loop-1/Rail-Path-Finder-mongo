    ...
    */
    async function recur(prev, curr, dest, coll, a) {
        ...
        console.log("c ",curr,"d ",dest,"coll ",coll);
            let arr = [];
            const r1 = await client.db('metro4').collection(coll).findOne({"Name":curr});
            console.log("r = ",r1);
            arr = r1.Link;
            console.log("arr = ",arr);
            ...
            for(let i=0; i<arr.length; i++) {
                ...
                let n = await client.db('metro4').collection(coll).findOne({"_id":arr[i]});
                console.log("n = ",n);
                let neighbor = n.Name;
                ...
                if(neighbor == prev) {
                    ...
                    console.log("dest = ",dest);
                    console.log("d = ",curr);
                    const r2 = await client.db('metro4').collection(coll).findOne({"Name":dest});
                    a.push(r2.Name);
                    const r3 = await client.db('metro4').collection(coll).findOne({"Name":curr});
                    a.push(r3.Name);
                    return 1;
                }
                else {
                    if(await recur(curr, neighbor, dest,coll, a )) {
                        console.log("h = ",curr);
                        const r4 = await client.db('metro4').collection(coll).findOne({"Name":curr});
                        a.push(r4.Name);
                        return 1;
                    }
                }
